...
const GET = async () => {
  try {
    ...
    const insights = {
      ...
      common_changes: Object.entries(changes.patterns || {}).map(([pattern, data]) => ({
        pattern,
        count: data.count || 0,
        keywords: data.keywords || []
      })).filter((p) => p.count > 0).sort((a, b) => b.count - a.count).slice(0, 10),
      recent_changes: changes.recent_changes?.slice(-10).reverse() || [],
      top_agents: Object.entries(stats.top_agents || {}).map(([agent, count]) => ({ agent, count })).sort((a, b) => b.count - a.count).slice(0, 5),
